...
export async function initDatabase() {
  ...
  try {
    console.log('[MySQL] Ensuring tables exist in ashastd24_ashaal...');
    ...
    if (prodRows[0].count === 0) {
      console.log('[MySQL] Products table is empty, seeding initial catalog...');
    ...
    if (bannerRows[0].count === 0) {
      console.log('[MySQL] Banners table is empty, seeding hero banners...');
    ...
    if (userRows[0].count === 0) {
      console.log('[MySQL] Users table is empty, seeding initial users...');
    ...
    if (orderRows[0].count === 0) {
      console.log('[MySQL] Orders table is empty, seeding initial orders...');
    ...
    if (visitorRows[0].count === 0) {
      console.log('[MySQL] Visitors table is empty, seeding initial visitors...');
      await seedVisitors(connection);
    }

    console.log('[MySQL] Database tables and initial data ready!');
  } catch (err) {
...
export async function seedProducts(connection?: mysql.PoolConnection) {
  ...
  try {
    ...
    console.log(`[MySQL] Seeded ${PRODUCTS_DATA.length} products`);
  } finally {
...
export async function seedBanners(connection?: mysql.PoolConnection) {
  ...
  try {
    ...
    console.log(`[MySQL] Seeded ${HERO_BANNERS.length} banners`);
  } finally {
    if (!connection) conn.release();
  }
}
...
export async function seedUsers(connection?: mysql.PoolConnection) {
  ...
  try {
    ...
    console.log(`[MySQL] Seeded ${INITIAL_SEED_USERS.length} users`);
  } finally {
    if (!connection) conn.release();
  }
}
...
export async function seedOrders(connection?: mysql.PoolConnection) {
  ...
  try {
    ...
    console.log(`[MySQL] Seeded ${INITIAL_ORDERS.length} orders`);
  } finally {
    if (!connection) conn.release();
  }
}
...
export async function seedVisitors(connection?: mysql.PoolConnection) {
  ...
  try {
    ...
    console.log(`[MySQL] Seeded ${INITIAL_VISITORS.length} visitor logs`);
  } finally {
    if (!connection) conn.release();
  }
}
